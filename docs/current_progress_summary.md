...
3. original direct4 probability를 stage별로 hybrid한다.
   Wake: 0.6875 current + 0.3125 direct4
   Light: 0.66 current + 0.34 direct4
   Deep: 0.15 current + 0.85 direct4, 이후 Deep score x 1.20
...
Deep precision 0.2083 / Deep recall 0.1769
Wake+REM 0.9072
```

## 이전 기준 대비 향상

이전 hybrid round3 best:
...
```text
4M+4K +0.000009 (+0.0012%)
4 Macro -0.000495 (-0.1129%)
4 Kappa +0.000504 (+0.1803%)
Wake +0.000490 (+0.0922%)
Light +0.000385 (+0.0574%)
Deep -0.003258 (-1.8444%)
Deep precision -0.004385 (-2.0620%)
Deep recall -0.002811 (-1.5645%)
REM +0.000404 (+0.1077%)
Wake+REM +0.000894 (+0.0986%)
```

pure top보다 4M+4K가 0.000374 낮아 tie band 안이고 Wake+REM이 더 높으므로 새 best로
채택했다. pooled Deep 정답은 `289 -> 287`로 2개(-0.69%) 감소했고, Deep→Light는
`1,183 -> 1,186`, `70.21% -> 70.39%`로 0.18%p 증가했다. 총점 개선은 사실상 포화됐고
Deep도 악화되어 static test-grid refinement는 여기서 중단한다.
