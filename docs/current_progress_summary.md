...
```text
current 4-role same-split ensemble + original direct4 stage-wise hybrid
...
5. original direct4: 1 checkpoint

outer split 하나의 실제 hybrid는 총 25 checkpoints다.
...
3. original direct4 probability를 stage별로 hybrid한다.
   Wake: 0.70 current + 0.30 direct4
   Light: 0.76 current + 0.24 direct4
   Deep: 0.05 current + 0.95 direct4, 이후 Deep score x 1.30
   REM: 1.00 current + 0.00 direct4
```
...
Deep precision 0.2086 / Deep recall 0.1847
Wake+REM 0.9052
```

## 이전 기준 대비 향상

이전 hybrid round1 best:
...
```text
4M+4K +0.0087 (+1.2352%)
4 Macro +0.0051 (+1.1880%)
4 Kappa +0.0036 (+1.3094%)
Wake +0.0074 (+1.4082%)
Light -0.0046 (-0.6775%)
Deep +0.0184 (+11.5637%)
Deep precision -0.0113 (-5.1173%)
Deep recall +0.0341 (+22.6781%)
REM -0.0006 (-0.1679%)
Wake+REM +0.0067 (+0.7500%)
```

4M+4K 차이가 0.0005보다 훨씬 크고 Wake+REM과 Deep도 함께 상승해 새 best로 채택했다.
pooled Deep 정답은 `242 -> 298`로 56개(+23.14%) 증가했고, Deep→Light는
`1,230 -> 1,173`, `73.00% -> 69.61%`로 3.38%p 감소했다.
...
```text
current + original direct4 hybrid Deep refinement round3
```

목적:

```text
round2 pure top은 Light 0.24 상단과 Deep alpha 0.95 하단에 걸렸고, Deep alpha 1.00/Light 0.18
ridge와 4M+4K 차이가 0.00004뿐이다. 두 ridge를 함께 덮어 Light/Deep alpha/gain 상호작용을
세밀하게 탐색한다. Wake 0.30과 gain 1.30은 내부 최적점 주변을 좁게 유지하고 REM은 0으로 고정한다.
```

grid:

```text
Wake alpha: 0.25~0.35
Light alpha: 0.16~0.34
Deep alpha: 0.80~1.00
REM alpha: 0.00
Deep gain: 1.15~1.45
총 2,205개 + alpha=0 baseline
```
...
```text
/content/drive/MyDrive/SSE_outputs/fusion4_same_split_ensemble_plus_direct4_original_hybrid_deep_refine_round3_context20_h64_summary.json
```

비교 포인트:

```text
1. round2 best 4M+4K 0.7169를 정확히 포함하고 재현하는지
2. pure 4M+4K top과 0.0005 tie-rule selected 후보
3. round2 best 대비 모든 metric의 절대/상대 변화율
4. Deep gain에 따른 precision/recall/F1과 pooled Deep→Light 변화
5. Light/Deep alpha가 다시 edge면 해당 방향을 추가 확장
```

round3 뒤에도 variance가 크면 `original direct4`의 same-split multi-init ensemble을 만들어
specialist seed variance를 줄인 뒤 hybrid를 다시 탐색한다.
...
hybrid_w0.30_li0.24_d0.95_rem0.00_dg1.30
3-seed 평균은 4M 0.4382 / 4K 0.2787 / 4M+4K 0.7169,
Wake 0.5306 / Light 0.6700 / Deep 0.1776 / REM 0.3745 / Wake+REM 0.9052다.
round1 best 대비 4M+4K +1.2352%, Deep +11.5637%, Wake+REM +0.7500%다.
다음 실험은 두 near-tied Light/Deep ridge를 덮는 hybrid refinement round3다.
Colab에서는 git pull 후 scripts/run_four_model_direct4_hybrid_deep_refinement_round3_colab.sh를 실행하면 돼.
결과 summary JSON을 받으면 round2 best 재현, pure top/tie-rule selected, round2 best 대비
4M+4K/Wake+REM/Light/Deep의 절대/상대 변화율을 비교하고 새 best 및 다음 방향을 결정해줘.
```
