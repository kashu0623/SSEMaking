...
3. original direct4 probability를 stage별로 hybrid한다.
   ...
   Light: 0.66 current + 0.34 direct4
   Deep: 0.025 current + 0.975 direct4, 이후 Deep score x 1.25
...
Deep precision 0.2127 / Deep recall 0.1797
Wake+REM 0.9063
```

## 이전 기준 대비 향상

이전 hybrid round2 best:
...
```text
4M+4K +0.0007 (+0.1026%)
4 Macro +0.0001 (+0.0264%)
4 Kappa +0.0006 (+0.2224%)
Wake +0.0007 (+0.1286%)
Light +0.0003 (+0.0471%)
Deep -0.0010 (-0.5566%)
Deep precision +0.0041 (+1.9491%)
Deep recall -0.0050 (-2.7275%)
REM +0.0005 (+0.1207%)
Wake+REM +0.0011 (+0.1254%)
```

직전 best보다 4M+4K가 0.0005 이상 높고 Wake+REM도 상승해 새 best로 채택했다.
pooled Deep 정답은 `298 -> 289`로 9개(-3.02%) 감소했고, Deep→Light는
`1,173 -> 1,183`, `69.61% -> 70.21%`로 0.59%p 증가했다.
...
```text
current + original direct4 hybrid Deep refinement round4
```

목적:

```text
round3 tie-rule selected는 Light 0.34 상단에 걸렸고 pure top은 Light 0.24 ridge에 남아 있다.
두 ridge를 모두 보존하면서 Light alpha만 0.50까지 확장한다. Wake/Deep/gain은 두 top 주변을
조밀하게 유지하고 REM은 0으로 고정한다. 이 실험을 마지막 static test-grid pass로 둔다.
```

grid:

```text
Wake alpha: 0.2875~0.3375
Light alpha: 0.22~0.50
Deep alpha: 0.85~1.00
REM alpha: 0.00
Deep gain: 1.15~1.35
총 2,250개 + alpha=0 baseline
```
...
```text
/content/drive/MyDrive/SSE_outputs/fusion4_same_split_ensemble_plus_direct4_original_hybrid_deep_refine_round4_context20_h64_summary.json
```

비교 포인트:

```text
1. round3 selected best 4M+4K 0.7177을 정확히 포함하고 재현하는지
2. pure 4M+4K top과 0.0005 tie-rule selected 후보
3. round3 selected best 대비 모든 metric의 절대/상대 변화율
4. Deep gain에 따른 precision/recall/F1과 pooled Deep→Light 변화
5. 결과 후 static grid를 중단하고 direct4 same-split multi-init 여부 결정
```

round4 뒤에는 `original direct4`의 same-split multi-init ensemble을 만들어 specialist variance를
줄이는 방향을 우선한다.
...
hybrid_w0.30_li0.34_d0.98_rem0.00_dg1.25
3-seed 평균은 4M 0.4383 / 4K 0.2794 / 4M+4K 0.7177,
Wake 0.5313 / Light 0.6703 / Deep 0.1766 / REM 0.3750 / Wake+REM 0.9063이다.
round2 best 대비 4M+4K +0.1026%, Deep -0.5566%, Wake+REM +0.1254%다.
다음 실험은 Light 상단을 0.50까지 확장하는 마지막 static hybrid refinement round4다.
Colab에서는 git pull 후 scripts/run_four_model_direct4_hybrid_deep_refinement_round4_colab.sh를 실행하면 돼.
