...
- `R`은 `REM`으로 매핑
- 파일별 non-`P` stage transition의 dominant `row % 3000` offset을 추정
- 해당 offset 기준으로 3,000-row window를 만들고, window 내부 label이 하나로 고정된 경우만 epoch로 사용
- mixed label window와 `P` window는 제외
