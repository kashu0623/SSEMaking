...
# Final half-step refinement of the round4 ridge. Preserve the selected, pure
# score top, and best-Deep candidate before moving back to model-level changes.
...
summary_json="${OUTPUT_ROOT}/fusion4_direct4_classwise_source_blend_hybrid_round5_context${CONTEXT_EPOCHS}_h${HIDDEN_SIZE}_summary.json"
PYTHONPATH=src "${PYTHON_BIN}" -m sse_sleep.evaluate_direct4_source_blend_hybrid \
  ...
  --wake-alphas 0.18125,0.1875,0.19375,0.20,0.20625,0.2125,0.21875,0.225,0.23125 \
  --light-alphas 0.5125,0.51875,0.525,0.53125,0.5375,0.54375,0.55 \
  --deep-alphas 0.8125,0.81875,0.825,0.83125,0.8375,0.84375,0.85,0.85625,0.8625,0.86875,0.875,0.88125,0.8875 \
  --rem-alphas 0.00 \
  --deep-gains 1.15,1.15625,1.1625,1.16875,1.175,1.18125,1.1875,1.19375,1.20,1.20625,1.2125 \
  --reference-source-betas 0.00,0.00,0.25,0.50 \
  --reference-hybrid-alphas 0.20625,0.525,0.8625,0.00 \
  --reference-deep-gain 1.20 \
  --archive-top 60 \
  --out-json "${summary_json}"

echo "=== Direct4 classwise source blend hybrid round5 complete: ${summary_json} ==="
