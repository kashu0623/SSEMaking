...
# Use RUN_TAG when changing SELECTION_METRIC or other settings to avoid
# overwriting earlier run directories.
...
variant_args() {
  ...
  case "${variant}" in
    remaux_w01)
      ...
      ;;
    remaux_w005)
      echo "--aux-head rem --aux-weight 0.05"
      ;;
    remaux_w02)
    ...
    deeprem_w01)
      ...
      ;;
    deeprem_w005)
      echo "--aux-head deep_rem --aux-weight 0.05"
      ;;
    deeprem_w02)
