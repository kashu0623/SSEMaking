...
def v7_b1_hr_std_iqr_imputed_disabled(feature: str) -> bool:
    base, _, _ = split_temporal_feature(feature)
...
def build_feature_map(
    ...
    if hr_ibi_feature_source == "v2":
        for signal in ("hr", "ibi"):
            for suffix in ("mean", "std", "median", "iqr", "min", "max", "slope"):
                feature_map[f"{signal}_{suffix}"] = f"{signal}_{suffix}_v2"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--hr-ibi-feature-source",
        choices=HR_IBI_FEATURE_SOURCES,
        default="current",
        help=(
            "current maps DreamT HR/IBI features to raw peak-derived features. "
