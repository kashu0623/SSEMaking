...
# Focused follow-up after focal gamma 1.0 and 2.0:
# test the middle point, gamma 1.5, on the current default LSTM setup.
...
for seed in "${SEEDS[@]}"; do
  ...
  echo "=== Train LSTM inverse focal gamma 1.5, seed ${seed} ==="
  PYTHONPATH=src python -m sse_sleep.train_lstm \
    --npz "${npz_path}" \
    --out-dir "${OUTPUT_ROOT}/lstm_temporal_context${CONTEXT_EPOCHS}_h${HIDDEN_SIZE}_inverse_focal_g15_seed${seed}" \
...
echo "=== Focal gamma 1.5 jobs complete ==="
