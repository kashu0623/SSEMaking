...
# No-training audit of N3 probabilities from the current same-split ensemble best.
# Thresholds are selected independently on each outer validation split and then
# applied unchanged to its test split.
...
summary_json="${OUTPUT_ROOT}/fusion4_same_split_init_ensemble_deep_probability_audit_context${CONTEXT_EPOCHS}_h${HIDDEN_SIZE}_summary.json"

PYTHONPATH=src "${PYTHON_BIN}" -m sse_sleep.evaluate_deep_probability_audit \
...
echo "=== Deep probability audit complete: ${summary_json} ==="
