...
# Refine the hybrid ridge shared by the round2 pure top, tie-rule selected, and
# prior best while fixing the direct4 source blend they all use.
...
summary_json="${OUTPUT_ROOT}/fusion4_direct4_classwise_source_blend_hybrid_round3_context${CONTEXT_EPOCHS}_h${HIDDEN_SIZE}_summary.json"
PYTHONPATH=src "${PYTHON_BIN}" -m sse_sleep.evaluate_direct4_source_blend_hybrid \
  ...
  --wake-alphas 0.125,0.1375,0.15,0.1625,0.175,0.1875,0.20 \
  --light-alphas 0.50,0.525,0.55,0.575,0.60 \
  --deep-alphas 0.75,0.775,0.80,0.825,0.85,0.875,0.90,0.925,0.95 \
  --rem-alphas 0.00 \
  --deep-gains 1.10,1.125,1.15,1.175,1.20,1.225,1.25,1.275,1.30 \
  --reference-source-betas 0.00,0.00,0.25,0.50 \
  --reference-hybrid-alphas 0.15,0.55,0.80,0.00 \
  --reference-deep-gain 1.25 \
  --archive-top 60 \
  --out-json "${summary_json}"

echo "=== Direct4 classwise source blend hybrid round3 complete: ${summary_json} ==="
